...
export default function PVCItemsPage() {
  ...
  useEffect(() => {
    ...
    const fetchItems = async () => {
      setLoading(true);
      try {
        const response = await fetch('/api/inventory/category/pvc');
        if (!response.ok) {
          throw new Error('Failed to fetch PVC items');
        }
        const data = await response.json();
        
        // Calculate statistics
        const totalValue = data.data.reduce((sum, item) => sum + (item.price * item.quantity), 0);
        const lowStockItems = data.data.filter(item => item.quantity < item.reorderPoint).length;
        const uniqueTypes = new Set(data.data.map(item => item.material)).size;
