    ...
    if (typeof window !== 'undefined') {
      ...
      if (storedItems) {
        ...
        if (Array.isArray(parsedItems)) {
          // Filter to only include safety items
          const safetyItems = parsedItems.filter(item => 
            item.category === 'safety' || 
            item.category === 'Safety' ||
            (item.name && item.name.toLowerCase().includes('safety'))
          );
          
          console.log(`Found ${safetyItems.length} safety items in localStorage`);
          return safetyItems.map(item => ({
            ...
            image: item.image || '/image/safety-placeholder.jpg',
...
export default function SafetyPage() {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/inventory/category/safety');
      
      if (!response.ok) {
        throw new Error('Failed to fetch safety items');
      }
      ...
      const normalizedItems = itemsArray.map(item => ({
        ...
        id: item._id || item.id,
        sku: item.sku || item.itemCode || `SAFE-${Math.floor(Math.random() * 100000)}`,
        name: item.name || item.itemName || 'Safety Item',
        ...
        price: Number(item.price || 0),
        image: item.image || 'https://media.istockphoto.com/id/1371686286/photo/helmets-for-construction-workers-engineers-technicians-inspectors-who-work-on-construction.jpg?s=2048x2048&w=is&k=20&c=NDkZbQ0tw8Siyr8NhRmrtFonqv1nNGFBoy5SltZ_wag=',
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h1 className="text-3xl font-bold text-black mb-2">Safety Equipment Inventory</h1>
                <p className="text-gray-800">Manage and track your safety equipment inventory</p>
              </div>
              ...
              <div className="relative h-48 bg-gray-100">
                <img
                  ...
                  onError={(e) => {
                    ...
                    e.target.src = 'https://media.istockphoto.com/id/1371686286/photo/helmets-for-construction-workers-engineers-technicians-inspectors-who-work-on-construction.jpg?s=2048x2048&w=is&k=20&c=NDkZbQ0tw8Siyr8NhRmrtFonqv1nNGFBoy5SltZ_wag=';
                  }}
                />
        ...
        {loading && (
          <div className="text-center py-12">
            ...
            <p className="mt-4 text-gray-500">Loading safety equipment...</p>
          </div>
        )}
      </div>
    </div>
