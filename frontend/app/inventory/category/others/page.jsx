    ...
    if (typeof window !== 'undefined') {
      ...
      if (storedItems) {
        ...
        if (Array.isArray(parsedItems)) {
          // Filter to only include other items
          const otherItems = parsedItems.filter(item => 
            item.category === 'others' || 
            item.category === 'Others' ||
            (item.name && item.name.toLowerCase().includes('other'))
          );
          
          console.log(`Found ${otherItems.length} other items in localStorage`);
          return otherItems.map(item => ({
            ...
            image: item.image || 'https://img.freepik.com/free-psd/elegant-chrome-bathroom-faucet-modern-design-luxury-style_409983930.jpg?t=st=1745953652~exp=1745957252~hmac=9ddabba7c879fe33679900e8c38e3bead006e15e02f8e93edce907e618f90e3c&w=996',
...
export default function OthersPage() {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/inventory/category/others');
      ...
      const normalizedItems = itemsArray.map(item => ({
        ...
        id: item._id || item.id,
        sku: item.sku || item.itemCode || `OTHER-${Math.floor(Math.random() * 100000)}`,
        name: item.name || item.itemName || 'Other Item',
        ...
        >
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h1 className="text-3xl font-bold text-black mb-2">Other Items Inventory</h1>
                <p className="text-gray-800">Manage and track your other items inventory</p>
              </div>
              ...
              <div className="relative h-48 bg-gray-100">
                <img
                  ...
                  onError={(e) => {
                    ...
                    e.target.src = 'https://img.freepik.com/free-psd/elegant-chrome-bathroom-faucet-modern-design-luxury-style_409983930.jpg?t=st=1745953652~exp=1745957252~hmac=9ddabba7c879fe33679900e8c38e3bead006e15e02f8e93edce907e618f90e3c&w=996';
                  }}
                />
